        ...
        switch (ev.*) {
            ...
            .definition => |*def| blk: {
                ...
                for (def.fields.items) |field| {
                    end_of_tmp_buf += (try std.fmt.bufPrint(tmp_buf[end_of_tmp_buf..], "{s}: {s}, ", .{ func(field.field), defs.FitBaseType.toString(field.typ) })).len;
                }
                std.log.debug("got def of type {s}. {s}", .{ defs.MesgNum.toString(def.global_msg_type), tmp_buf[0..end_of_tmp_buf] });
            },
            .data => |*data| blk: {
                defer data.deinit();
